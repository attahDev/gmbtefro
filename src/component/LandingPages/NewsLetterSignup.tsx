...
export default function NewsletterSignup() {
    ...
    return (
        <section className="relative flex w-full justify-center overflow-hidden bg-[#FFD700] py-12 sm:py-16 md:py-24 lg:pb-32">
            <div className="relative w-full max-w-[1400px] px-4 sm:px-6 md:px-12">
                <div className="relative mx-auto w-full max-w-[1135px] overflow-hidden lg:overflow-visible">
                    {/* Blue Card */}
                    <motion.div
                        ...
                        viewport={{ once: true }}
                        className="relative z-20 h-auto w-full bg-[#001F3F] p-6 text-left text-white shadow-xl sm:p-12 lg:min-h-[600px] lg:w-[820px] lg:p-16"
                    >
                        <h4 className="mb-3 text-[18px] tracking-wide text-white">
                            Stay in the Loop
                        </h4>
                        <h2 className="mb-6 text-2xl font-bold leading-tight sm:mb-8 sm:text-[36px]">
                            Sign Up For Our <br /> Newsletter
                        </h2>
                        <p className="font-montserrat mb-8 max-w-lg text-[14px] leading-relaxed text-[#FFFFFF99] sm:mb-10 sm:text-[18px]">
                        ...
                        {success && (
                            <div className="mb-6 inline-flex max-w-md items-start gap-3 rounded-xl border border-emerald-400/40 bg-emerald-500/15 px-4 py-3 text-left text-emerald-100">
                                <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-400 text-[#001F3F]">
                                    <Check className="h-4 w-4" strokeWidth={3} />
                                </span>
                                <div>
                                    <p className="font-semibold text-white">You&apos;re on the list!</p>
                                    <p className="mt-0.5 text-sm text-emerald-100/90">
                                        Thanks for joining. Watch your inbox for the latest events,
                        ...
                        <div className="flex max-w-[500px] flex-col gap-6">
                            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                                <div>
                                    ...
                                    <input
                                        ...
                                        disabled={success}
                                        aria-invalid={!!error}
                                        className="w-full rounded-xl border-0 bg-[#132E57] px-4 py-3 text-white placeholder:text-[#FFFFFF26] transition focus:outline-none focus:ring-2 focus:ring-[#C1283C] disabled:opacity-60"
                            ...
                            {error && (
                                <div
                                    role="alert"
                                    className="flex items-center gap-2.5 rounded-lg border border-[#C1283C]/40 bg-[#C1283C]/15 px-3 py-2.5"
                                >
                                    <AlertCircle className="h-4 w-4 shrink-0 text-[#FF8A96]" strokeWidth={2.5} />
                                    <p className="min-w-0 flex-1 text-sm leading-snug text-white/90">{error}</p>
                                    <button
                                        type="button"
                            ...
                            <motion.button
                                ...
                                whileHover={success ? undefined : { scale: 1.05 }}
                                whileTap={success ? undefined : { scale: 0.97 }}
                                className={`flex w-full items-center justify-center gap-3 rounded-xl px-6 py-4 text-base text-white shadow-lg transition-all duration-300 disabled:cursor-not-allowed ${
                                    success
                                        ? "bg-emerald-600 hover:bg-emerald-600"
                                        : "bg-[#C1283C] hover:bg-[#A31F32] disabled:opacity-50"
                                }`}
                            >
                                {loading ? (
                                    "Subscribing..."
                    ...
                    <motion.div
                        ...
                        transition={{ duration: 0.8, ease: "easeOut" }}
                        viewport={{ once: true }}
                        className="absolute bottom-[-40px] right-12 z-30 hidden h-[550px] w-[420px] lg:block"
                    >
                        <img
                    ...
                    {/* Decorative pattern — clipped by section overflow */}
                    <div
                        aria-hidden
                        className="pointer-events-none absolute bottom-[-120px] right-[-180px] hidden rotate-[-20deg] select-none opacity-10 sm:block lg:bottom-[-280px] lg:right-[-280px]"
                    >
